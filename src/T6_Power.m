...
%-----------------------------------------------------------------
fig6_1 = readtable(Table, ...
    Sheet="T6", Range=fig6_1_range);

X = fig6_1.bistmdelta;
Y = fig6_1.beta;

beta_Fnc = Interpolation_Fnct(X, Y);
beta1 = beta_Fnc(bistm1 / delta);
beta2 = beta_Fnc(bistm2 / delta);

beta0 = [beta1, beta2];

clear("fig6_1", "beta_Fnc", "beta1", "beta2");
%-----------------------------------------------------------------
...
fprintf("\n======================[ %s ]=======================\n", rez);
fprintf("\t\t%s Rand: %.2f %s %s 5 %s", ...
    rez, 100*abs(randamentul_estimat - randamentul_nominal)/randamentul_estimat, "%", sign, "%");
fprintf("\n=====================================================\n\n");


fprintf("[ PASS ] T6 Script finished succesfully!\n");
